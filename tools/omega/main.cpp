...
bool GeneralTextCompare(
    ...
    char cAltTerminator =
        '\0'  // For function names, for example, you can stop at the first '('
    ) {
  bool bMatch = true;
...
void parse_command_line(int argc, char **argv, CompilerParameters *parameters) {
  LOG(DEBUG);
  ArgvParser cmd;
  // init
  ...
  cmd.defineOption("target",
                   ...
                   " one of three options:/target:appcontainerexe, "
                   "--target=exe, --target=library.",
                   ArgvParser::OptionRequiresValue);
  ...
  for (int i = 0; i < cmd.arguments(); i++) {
    // todo: parse out '/' or '\' to find out if we need to search inside some
    // directory.
...
int main(int argc, char **argv) {
  ...
  for (string file_name : *parameters.input_files()) {
    ...
    if (errors.size() > 0) {
      for (auto e : errors) {
        std::cerr << e->location()->to_string() << ": error: " << e->message()
                  << std::endl;
      }
    }
  }
  ...
  if (total_errors == 0 && parser_success) {
    ...
    for (auto x : result->errors()) {
      if (x->has_location())
        std::cerr << x->location()->to_string() << ": error: " << x->message()
                  << std::endl;
      else
