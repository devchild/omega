...
/** Provides parsing and storage of POSIX-like command line arguments (argc,
* argv).
* To use this class for CLI-option parsing, first define a set of valid options
* using
* the defineOption() method. An option can have several attributes: it can be
* required
* itself (its missing is considered as an error) and/or it can require a value.
* Options with optional values can be realized by defining the option to not
* need a
* value and use the syntax '--option=value' or '-o=value' on the command line.
* Every option can have different alternative labels; see
* defineOptionAlternative().
* A usage description (see usageDescription()) can be generated from the set of
* defined options (also see the
* addErrorCode(), setIntroductoryDescription() and setHelpOption() methods).
* \note The implemented parsing algorithm requires that all options have to be
* given at
* the beginning of the command line. Everything on the commandline after the
* first
...
*
* cmd.defineOption("foo", ArgvParser::OptionRequiresValue, "Fooishness. Default
* value: 0");
*
...
class ArgvParser {
 ...
 private:
  ...
  /** General description to be returned as first part of the generated help
   * page. */
 ...
 *  4. first short option character must not be a digit (to distinguish negative
 * numbers)
 */
bool isValidOptionString(const std::string& _string);

/** Returns whether the given string is a valid (correct syntax) long option
 * string.
 * It has to fullfill the following criteria:
