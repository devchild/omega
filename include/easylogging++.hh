...
#endif  // ((ELPP_OS_LINUX || ELPP_OS_MAC || ELPP_OS_FREEBSD || ELPP_OS_SOLARIS)
        // && (!ELPP_OS_WINDOWS))
// Android
...
#endif  // (!ELPP_MINGW && !ELPP_COMPILER_CLANG) ||
...
#endif  // ELPP_COMPILER_GCC || ELPP_COMPILER_CLANG || ELPP_COMPILER_INTEL ||
        // (ELPP_COMPILER_MSVC && _MSC_VER >= 1800)
...
const struct {
  ...
  const base::type::char_t* unit;
    ...
    {1000.0f, ELPP_LITERAL("mis")},   {1000.0f, ELPP_LITERAL("ms")},
...
/// @brief Gets value of const char* but if it is nullptr, a string nullptr is
/// returned
...
class Str : base::StaticClass {
 public:
  ...
  static inline std::string& replaceAll(std::string& str, char replaceWhat,
                                        char replaceWith) {
  ...
  static inline std::string& replaceAll(
      std::string& str, const std::string& replaceWhat,  // NOLINT
  ...
  static inline char* convertAndAddToBuff(std::size_t n, int len, char* buf,
                                          const char* bufLim,
  ...
  static inline char* addToBuff(const char* str, char* buf,
                                const char* bufLim) {
...
class DateTime : base::StaticClass {
 public:
  ...
  /// be used to determine current millisecond.
  ///
  ...
  static inline std::string getDateTime(
      const char* format, const base::MillisecondsWidth* msWidth) {
  ...
  /// timestampUnit
  static inline unsigned long long getTimeDifference(
      ...
      if (*format == base::consts::kFormatSpecifierChar) {
        switch (*++format) {
          ...
          case 'A':  // Day of week (long)
            ...
            continue;
          case 'M':  // month
            buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_mon + 1, 2,
                                                        buf, bufLim);
            continue;
          case 'b':  // month (short)
            buf = base::utils::Str::addToBuff(
          ...
          case 'z':  // milliseconds
          case 'g':
            buf = base::utils::Str::convertAndAddToBuff(msec, msWidth->m_width,
                                                        buf, bufLim);
            continue;
          case 'F':  // AM/PM
            buf = base::utils::Str::addToBuff(
    ...
    for (int i = 1; i < m_argc; ++i) {
      ...
      if (v != nullptr && strlen(v) > 0) {
        ...
        if (hasParamWithValue(key.c_str())) {
          ELPP_INTERNAL_INFO(
              1, "Skipping [" << key << "] arg since it already has value ["
...
/// unregisterAll() and deepCopy(const AbstractRegistry<T_Ptr, Container>&) and
/// write registerNew() method according to container
...
class Configurations
 ...
 public:
  ...
  inline bool parseFromFile(const std::string& configurationFile,
    ...
    // want to pass this
    // check and if assertion is enabled we will have values re-assigned any
    // way.
  ...
  /// @param configurationType Type of configuration to check existence for.
  inline bool hasConfiguration(Level level,
  ...
  inline void set(Level level, ConfigurationType configurationType,
                  const std::string& value) {
    base::threading::ScopedLock scopedLock(lock());
   ...
   private:
    ...
    static bool parseLine(std::string* line, std::string* currConfigStr,
      ...
      *line = base::utils::Str::trim(*line);
      ...
      ELPP_ASSERT(*currLevel != Level::Unknown, "Unrecognized severity level ["
                                                    << *currLevelStr << "]");
...
class TypedConfigurations : public base::threading::ThreadSafe {
 public:
  ...
  inline const base::MillisecondsWidth& millisecondsWidth(
 ...
 private:
  ...
  template <typename Conf_T>
  inline Conf_T& getConfigByRef(Level level, std::map<Level, Conf_T>* confMap,
                                const char* confName) {
    base::threading::ScopedLock scopedLock(lock());
  ...
  void build(Configurations* configurations) {
    ...
    for (Configurations::const_iterator it = configurations->begin();
      ...
      if (conf->configurationType() == ConfigurationType::Enabled) {
        ...
        setValue(Level::Global, base::MillisecondsWidth(
...
class RegisteredHitCounters
 ...
 public:
  ...
  /// @return True if validation resulted in triggering hit. Meaning logs should
  /// be written everytime true is returned
  bool validateAfterN(const char* filename, unsigned long int lineNumber,
                      std::size_t n) {
    base::threading::ScopedLock scopedLock(lock());
    base::HitCounter* counter = get(filename, lineNumber);
    if (counter == nullptr) {
  ...
  /// @return True if validation resulted in triggering hit. Meaning logs should
  /// be written everytime true is returned
  bool validateNTimes(const char* filename, unsigned long int lineNumber,
                      std::size_t n) {
    base::threading::ScopedLock scopedLock(lock());
    base::HitCounter* counter = get(filename, lineNumber);
    if (counter == nullptr) {
...
class RegisteredLoggers : public base::utils::Registry<Logger, std::string> {
 public:
  ...
  virtual ~RegisteredLoggers(void) { flushAll(); }
...
class VRegistry : base::NoCopy, public base::threading::ThreadSafe {
 public:
  ...
  inline const std::map<std::string, base::type::VerboseLevel>& modules(
      void) const {
    return m_modules;
  }
...
class AsyncDispatchWorker : public base::IWorker,
 ...
 public:
  ...
  bool continueRunning(void) { return m_continueRunning; }
...
class LogDispatcher : base::NoCopy {
 public:
  ...
  void dispatch(void) {
    ...
    // We minimize the time of ELPP's lock - this lock is released after log is
    // written
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
  ...
  inline MessageBuilder& writeIterator(Iterator begin_, Iterator end_,
                                       std::size_t size_) {
    m_logger->stream() << ELPP_LITERAL("[");
    for (std::size_t i = 0;
...
class Writer : base::NoCopy {
 ...
 protected:
  ...
  void triggerDispatch(void) {
    if (m_proceed) {
      base::LogDispatcher(m_proceed, LogMessage(m_level, m_file, m_line, m_func,
                                                m_verboseLevel, m_logger),
                          m_dispatchAction)
          .dispatch();
    }
    if (m_logger != nullptr) {
...
class PErrorWriter : public base::Writer {
 public:
  PErrorWriter(
      Level level, const char* file, unsigned long int line, const char* func,
      base::DispatchAction dispatchAction = base::DispatchAction::NormalLog,
      base::type::VerboseLevel verboseLevel = 0)
...
template <typename T, typename... Args>
void Logger::log(Level level, const char* s, const T& value,
                 const Args&... args) {
  base::threading::ScopedLock scopedLock(lock());
 ...
 public:
  PerformanceTracker(
      ...
      const std::string& loggerId =
          std::string(ELPP_CURR_FILE_PERFORMANCE_LOGGER),
      bool scopedLog = true,
    ...
    // end of scope
    // or before calling checkpoint, we still depend on state of configuraton at
...
class StackTrace : base::NoCopy {
 public:
  ...
  class StackTraceEntry {
   public:
    StackTraceEntry(std::size_t index, const char* loc, const char* demang,
                    const char* hex, const char* addr) {
      m_index = index;
...
class Helpers : base::StaticClass {
 public:
  ...
  static inline void crashAbort(int sig, const char* sourceFile = "",
  ...
  static inline void logCrashReason(
...
class Loggers : base::StaticClass {
 public:
  ...
  static inline Logger* reconfigureLogger(
      Logger* logger, const Configurations& configurations) {
  ...
  static inline void setDefaultConfigurations(
      const Configurations& configurations,
  ...
  static inline std::vector<std::string>* populateAllLoggerIds(
...
// NOTE: no ELPP_INITI_BASIC_DECLR when sharing - causes double free corruption
// on external symbols
