...
namespace mlang {
	...
	public:
		SemanticPass(vector<CompilerError*> &errors) :
				m_errors(errors) {
		}
		...
		inline vector<CompilerError*> &errors() {
	...
	public:
		...
		LoadTypes(vector<CompilerError*> &errors) :
				SemanticPass(errors) {
		}
	...
	public:
		ResolveReferences(vector<CompilerError*> &errors,
						  std::map<std::string, CodeTypeDeclaration *> &type_declarations) :
	...
	SemanticAnalysis::SemanticAnalysis(const DomProvider &p) :
			m_provider(p) {

	}

	SemanticAnalysis::SemanticAnalysis(const SemanticAnalysis &orig) :
	...
	void SemanticAnalysis::analyse(CodeCompileUnit *compile_unit) {
