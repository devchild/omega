...
namespace mlang {
	CodeCompiler::CodeCompiler(DomProvider &p) :
			m_provider(p) {
	}
	...
	DomProvider &CodeCompiler::provider() {
...
/*	static int generateobj(std::shared_ptr<llvm::tool_output_file> Out, llvm::Module * module)
	...
	static int generateobj(llvm::raw_fd_ostream &out, llvm::Module *module) {
		...
		const llvm::Target *TheTarget = llvm::TargetRegistry::lookupTarget(TheTriple.getTriple(), Err);
		std::string MCPU, FeaturesStr;
		llvm::TargetMachine *machineTarget = TheTarget->createTargetMachine(TheTriple.getTriple(), MCPU, FeaturesStr,
																			Options);
	...
	void
	CodeCompiler::IncludeEmbeddedTypes(mlang::CodeCompileUnit *compile_unit) {
		LOG(TRACE);
		//
