...
namespace mlang {
    ...
    bool CodeMethodInvokeExpression::type_of(CodeObjectKind kind) {
        return kind == CodeObjectKind::CodeMethodInvokeExpression || CodeExpression::type_of(kind);
    }

    void 
    CodeMethodInvokeExpression::scope(CodeScope* scope) {
        CodeExpression::scope(scope);
        
        if (this->m_method != nullptr)
            this->m_method->scope(scope);
