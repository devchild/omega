...
namespace mlang {
    ...
    bool CodeCastExpression::type_of(CodeObjectKind kind) {
        return kind == CodeObjectKind::CodeCastExpression || CodeExpression::type_of(kind);
    }
    
    void 
    CodeCastExpression::scope(CodeScope* scope) {
        CodeExpression::scope(scope);
        
        if (this->target_type() != nullptr)
            this->target_type()->scope(scope);
