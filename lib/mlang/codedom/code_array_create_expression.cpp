...
namespace mlang {
    ...
    bool CodeArrayCreateExpression::type_of(CodeObjectKind kind) {
        return kind == CodeObjectKind::CodeArrayCreateExpression || CodeExpression::type_of(kind);
    }
	...
	CodeArrayCreateExpression::scope(CodeScope* scope) {
        ...
        if (this->m_size_expression != nullptr)
        	this->m_size_expression->scope(scope);
    }

    CodeExpression*
	CodeArrayCreateExpression::size_expression() {
    	return this->m_size_expression;
    }

    void
	CodeArrayCreateExpression::size_expression(CodeExpression* value) {
    	this->m_size_expression = value;
    }

    CodeTypeReference*
	CodeArrayCreateExpression::create_type() {
    	return this->m_create_type;
    }

    void
	CodeArrayCreateExpression::create_type(CodeTypeReference* value) {
