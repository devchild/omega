...
namespace mlang {
    ...
    MemberAttributes CodeMemberMethod::attributes() {
    	return this->m_attributes;
    }

    std::string CodeMemberMethod::id() {
        if (this->m_attributes == MemberAttributes::External || this->m_attributes == MemberAttributes::Public)
        	return this->name();
        else
	...
	bool CodeMemberMethod::type_of(CodeObjectKind kind) {
		return kind == CodeObjectKind::CodeMemberMethod || CodeTypeMember::type_of(kind);
	}
	...
	void CodeMemberMethod::scope(CodeScope* scope) {
		...
		if (this->m_return_type != nullptr)
			this->m_return_type->scope(scope);
