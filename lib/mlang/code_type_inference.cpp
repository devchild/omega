...
namespace mlang {
	...
	void MLangCodeTypeInference::on_visit(CodeBinaryOperatorExpression *object) {
		...
		params.push_back(
				(CodeTypeDeclaration *) inf_left.result()->user_data(UserDataKind::MLANG_RESOLVED_TYPE_DECLARATION));

		MLangCodeTypeInference inf_right;
		object->right()->accept(&inf_right);
		params.push_back(
				(CodeTypeDeclaration *) inf_right.result()->user_data(UserDataKind::MLANG_RESOLVED_TYPE_DECLARATION));
	...
	void MLangCodeTypeInference::on_visit(CodeMethodInvokeExpression *object) {
		...
		if (object->method()->target_object() != nullptr) {
			...
			param_types.push_back(
					(CodeTypeDeclaration *) param_inf.result()->user_data(
		...
		auto method = (CodeMemberMethod *) object->user_data(
				UserDataKind::MLANG_RESOLVED_MEMBER_METHOD); //object->resolve_method(object->method()->method_name(), &param_types);
	...
	void MLangCodeTypeInference::on_visit(CodeFieldReferenceExpression *object) {
		...
		auto target_object_type = (CodeTypeDeclaration *) exp_inf.result()->user_data(
				UserDataKind::MLANG_RESOLVED_TYPE_DECLARATION);
