...
namespace mlang {
		...
		if (this->m_result == nullptr) {
			for (auto x : *object->statements()) {
				if (x->type_of(CodeObjectKind::CodeMethodReturnStatement)) {
                    ...
                    this->m_result = CodeTypeInference::get_type_reference(return_statement);
				}
			}
		}
	}
	...
	CodeTypeReference *CodeTypeInference::get_type_reference(CodeObject *object) {
		...
		CodeTypeReference* ret = (CodeTypeReference* )object->user_data(UserDataKind::MLANG_INFERRED_TYPE_REFERENCE);
		if (ret != nullptr)
			return ret;
