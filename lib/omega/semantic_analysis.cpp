                ...
                for (auto member : *object->members()) {
                    if (member->type_of(CodeObjectKind::CodeMemberField)) {
                        ...
                        if (is_cyclic_member_field(object, field)) {
                            ...
                            if (field_type != nullptr && has_cyclic_members(object, field_type->members())) {
                                error(field->location(),
                                      "Struct member `" + object->name() + "."
                                      + member->name() + "' of type `"
                                      + field->type()->base_type()
            ...
            for (auto member : *members) {
                if (member->type_of(CodeObjectKind::CodeMemberField)) {
                    ...
                    if (is_cyclic_member_field(root, field)) {
                        ...
                        CodeTypeDeclaration * field_type = CodeResolver::resolve(field->type());
...
vector<mlang::CompilerError *> &
mlang::SemanticAnalysis::errors() {
    return this->m_errors;
}
