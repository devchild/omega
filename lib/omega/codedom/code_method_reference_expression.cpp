...
CodeTypeReferenceCollection& CodeMethodReferenceExpression::type_arguments() {
...
bool CodeMethodReferenceExpression::type_of(CodeObjectKind kind) {
  return kind == CodeObjectKind::CodeMethodReferenceExpression ||
         CodeExpression::type_of(kind);
}

void CodeMethodReferenceExpression::scope(CodeScope* scope) {
  CodeExpression::scope(scope);
  if (this->target_object() != nullptr) this->target_object()->scope(scope);

  for (auto x : this->type_arguments()) x->scope(scope);
}
}
