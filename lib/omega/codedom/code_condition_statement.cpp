...
CodeStatementCollection* CodeConditionStatement::true_statements() {
  return this->m_true_statements;
}

CodeStatementCollection* CodeConditionStatement::false_statements() {
  return this->m_false_statements;
}

CodeExpression* CodeConditionStatement::condition() {
  return this->m_condition;
}

void CodeConditionStatement::condition(CodeExpression* value) {
  this->m_condition = value;
}

bool CodeConditionStatement::type_of(CodeObjectKind kind) {
  return kind == CodeObjectKind::CodeConditionStatement ||
         CodeStatement::type_of(kind);
}

void CodeConditionStatement::accept(ICodeObjectVisitor* visitor) {
  visitor->visit(this);
}

void CodeConditionStatement::scope(CodeScope* scope) {
  ...
  for (auto x : *this->true_statements()) x->scope(current_scope);
  current_scope = current_scope->end_scope();
  // end true_statements

  // begin false_statements
  current_scope = current_scope->begin_scope();
  for (auto x : *this->false_statements()) x->scope(current_scope);
  current_scope = current_scope->end_scope();
