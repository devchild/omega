...
CodeMemberMethodCollection* CodeCompileUnit::methods() {
  return this->m_methods;
}

CodeTypeDeclarationCollection* CodeCompileUnit::types() {
  return this->m_types;
}

void CodeCompileUnit::accept(ICodeObjectVisitor* visitor) {
  visitor->visit(this);
}

bool CodeCompileUnit::type_of(CodeObjectKind kind) {
  return kind == CodeObjectKind::CodeCompileUnit || CodeObject::type_of(kind);
}

void CodeCompileUnit::scope(CodeScope* scope) {
