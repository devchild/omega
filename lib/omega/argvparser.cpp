...
string ArgvParser::optionValue(const string& _option) const {
  ...
  if (key == -1) {
    cerr << "ArgvParser::optionValue(): Requested value of an option the "
            "parser did not find or does not know."
         << endl;
    return ("");
  }
...
ArgvParser::ParserResults ArgvParser::parse(int _argc, char** _argv) {
  bool finished_options =
      false;  // flag whether an argument was found (options are passed)
    ...
    } else  // can be a long or multiple short options at this point
    {
      ...
      if (isValidLongOptionString(argument)) {
        ...
        if ((option2attribute.find(key)->second & OptionRequiresValue) &&
          ...
          {
            error_option = option;  // store the option that caused the error
            return (ParserMissingValue);  // the was no argument left although
                                          // we need a value
          }
      ...
      } else  // handle short options
      {
        ...
        if (splitOptionAndValue(argument, option, value)) {
          ...
          if (option.length() > 1) {
            error_option = option;  // store the option that caused the error
            return (ParserMalformedMultipleShortOption);  // return error code
                                                          // if option has more
                                                          // than one character
          }
          ...
          key = option2key.find(option)
                    ->second;  // get the key for the extracted option name
                ...
                // something like '-s 4'
        {
          ...
          if (argument.length() == 1)  // if a single short option
          {
            ...
            {
              error_option =
                  argument;  // store the option that caused the error
              return (ParserUnknownOption);  // return error code if not
            }
            key = option2key.find(argument)
                      ->second;  // get the key for the extracted option name
            ...
            if ((option2attribute.find(key)->second & OptionRequiresValue)) {
              if (i + 1 >= _argc)  // are there arguments left?
              {
                error_option =
                    argument;  // store the option that caused the error
                return (ParserMissingValue);  // the was no argument left
                                              // although we need a value
              }
              string temp = _argv[i + 1];  // get the next element
              ++i;  // increase counter now that we moved forward
          ...
          {
            unsigned int short_option_counter =
                0;  // position in the multiple short option string
            while (short_option_counter <
                   argument.length())  // parse the whole string
            {
              option =
                  argument[short_option_counter];  // get the option character

              if (!isDefinedOption(option))  // is this a known option
              {
                error_option =
                    option;  // store the option that caused the error
                return (ParserUnknownOption);  // return error code if not
              }
              key = option2key.find(option)
                        ->second;  // get the key for the extracted option name
...
string ArgvParser::usageDescription(unsigned int _width) const {
  ...
  if (max_key > 1)  // if we have some options
    usage +=
        formatString("Available options\n-----------------", _width) + "\n";
...
bool ArgvParser::defineOption(const string& _name, const string& _descr,
  ...
  if (isDefinedOption(_name)) {
    cerr << "ArgvParser::defineOption(): The option label equals an already "
            "defined option."
         << endl;
    return (false);
  }
  ...
  if (_name.length() == 1 && isDigit(_name[0])) {
    cerr << "ArgvParser::defineOption(): Digits as short option labels are not "
            "allowd."
         << endl;
    return (false);
  }
...
bool ArgvParser::defineOptionAlternative(const string& _original,
  ...
  if (!isDefinedOption(_original)) {
    cerr << "ArgvParser::defineOptionAlternative(): Original option label is "
  ...
  if (_alternative.length() == 1 && isDigit(_alternative[0])) {
    cerr << "ArgvParser::defineOptionAlternative(): Digits as short option "
            "labels are not allowd."
         << endl;
    return (false);
  }
  ...
  if (isDefinedOption(_alternative)) {
    cerr << "ArgvParser::defineOptionAlternative(): The alternative option "
            "label equals an already defined option."
         << endl;
    return (false);
  }
...
bool ArgvParser::setHelpOption(const string& _shortname,
  ...
  if (isDefinedOption(_shortname) || isDefinedOption(_longname)) {
    cerr << "ArgvParser::setHelpOption(): Short or long help option label "
            "equals an already defined option."
         << endl;
    return (false);
  }
...
bool CommandLineProcessing::expandRangeStringToUInt(
    const std::string& _string, std::vector<unsigned int>& _expanded) {
  list<string> tokens;
  // split string by delimiter
  splitString(tokens, _string, ",");
