...
CodeObject *mlang::CodeResolver::on_resolve(CodeVariableReferenceExpression *object) {
    ...
    while (current_scope != nullptr) {
        for (auto x:*current_scope->members()) {
            if (x->type_of(CodeObjectKind::CodeVariableDeclarationStatement)) {
                ...
                if (tmp->name() == object->variable_name())
                    return tmp;
            }
...
CodeObject *mlang::CodeResolver::on_resolve(CodeFieldReferenceExpression *object) {
    auto target_type_reference = CodeTypeInference::get_type_reference(object->target_object());
    auto target_type_declaration = CodeResolver::resolve(target_type_reference);

    for (auto x: *target_type_declaration->members()) {
        if (x->type_of(CodeObjectKind::CodeMemberField)) {
            auto tmp = (CodeMemberField *) x;
            if (tmp->name() == object->field_name())
                return tmp;
        }
    }
...
CodeTypeDeclaration *mlang::CodeResolver::on_resolve(CodeTypeReference *code_type_reference) {
...
CodeMemberMethod *mlang::CodeResolver::on_resolve(CodeMethodInvokeExpression *object) {
    ...
    if (object->method()->target_object() != nullptr) {
        auto object_method_reference_target_type =
                CodeTypeInference::get_type_reference(object->method()->target_object());
    ...
    for (auto method: methods) {
        ...
        // only method that are not part of a class definition can be called like 1._invoke_something()
        if (method_owner_type == nullptr && object->method()->target_object() != nullptr)
        ...
        while (ok
            ...
            auto method_declared_parameter_type = CodeResolver::resolve(method_declared_parameter->type());
            auto passed_parameter_type = CodeResolver::resolve(CodeTypeInference::get_type_reference(passed_parameter));
...
CodeTypeDeclaration *mlang::CodeResolver::resolve(CodeTypeReference *code_type_reference) {
    CodeTypeDeclaration * ret = (CodeTypeDeclaration *) code_type_reference->user_data(
...
CodeObject *mlang::CodeResolver::resolve(CodeVariableReferenceExpression *object) {
    auto ret = (CodeObject *) object->user_data(UserDataKind::MLANG_RESOLVED_VARIABLE);
    if (ret != nullptr)
        return ret;
    else {
...
CodeMemberMethod *mlang::CodeResolver::resolve(CodeMethodInvokeExpression *object) {
    auto ret = (CodeMemberMethod *) object->user_data(UserDataKind::MLANG_RESOLVED_METHOD);
    if (ret != nullptr)
        return ret;
    else {
        ret = on_resolve(object);
...
CodeBinaryOperatorType mlang::CodeResolver::get_operator_type(CodeMemberMethod* object) {
    for(auto attr:object->custom_attributes()) {
        ...
        if (!attr->arguments().front()->value()->type_of(CodeObjectKind::CodePrimitiveExpression))
            return CodeBinaryOperatorType::None;

        auto argument = ((CodePrimitiveExpression *) attr->arguments().front()->value());
        ...
        if (operator_name == "<<")
            return CodeBinaryOperatorType::ShiftLeft;
        if (operator_name == ">")
            return CodeBinaryOperatorType::GreaterThan;
        if (operator_name == ">>")
            return CodeBinaryOperatorType::ShiftRight;
        if (operator_name == "<=")
        ...
        if (operator_name == "^")
            return CodeBinaryOperatorType::ExlusiveOr;
...
CodeMemberMethod *mlang::CodeResolver::on_resolve(CodeBinaryOperatorExpression *object) {
    CodeScope *current_scope = object->scope();
    while (current_scope != nullptr) {
        for (auto x:*current_scope->members()) {
            if (x->type_of(CodeObjectKind::CodeMemberMethod)) {
                ...
                CodeBinaryOperatorType operatorType = CodeResolver::get_operator_type(method);
                if (operatorType == object->operator_())
                {
                    auto left_t = CodeResolver::resolve( CodeTypeInference::get_type_reference( object->left() ) );
                    auto right_t = CodeResolver::resolve( CodeTypeInference::get_type_reference( object->right() ) );

                    auto first_t = CodeResolver::resolve( CodeTypeInference::get_type_reference(
                            method->parameters()->front() ) );

                    auto last_t = CodeResolver::resolve( CodeTypeInference::get_type_reference(
                            method->parameters()->back() ) );

                    if (left_t == first_t && right_t == last_t)
                        return method;
                }
            }
        }
...
CodeMemberMethod *mlang::CodeResolver::resolve(CodeBinaryOperatorExpression *object) {
    auto ret = (CodeMemberMethod *) object->user_data(UserDataKind::MLANG_RESOLVED_METHOD);
    if (ret != nullptr)
        return ret;
    else {
